...
function RecursiveSearchForLink(elem, lvl) {
  $(elem).find('iframe').each(function (index) {
    ...
    if ( (!$(this).attr('src').match(/http\:\/\/dayt\.se/g))&&
         ($(this).attr('src').match(/http/g)) ) {
      return;
    } // reject iframes with no child iframes

    if ($(this).contents().find('iframe') [0] == undefined) {
      return;
    }
...
window.onload = function () {
  $('#navbar > font').append('<button onclick=\'DaytSEDownload()\'>Download</button>');
